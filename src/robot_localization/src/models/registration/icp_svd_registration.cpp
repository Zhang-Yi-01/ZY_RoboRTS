...
namespace robot_localization
{
    ...
    ICPSVDRegistration::ICPSVDRegistration(const YAML::Node &node) : input_target_kdtree_(new pcl::KdTreeFLANN<pcl::PointXYZ>())
    {
        // parse params:
    ...
    ICPSVDRegistration::ICPSVDRegistration(float max_corr_dist,
                                           float trans_eps,
    ...
    bool ICPSVDRegistration::ScanMatch(const CloudData::CLOUD_PTR &input_source, //  输入待配准点云
                                       const Eigen::Matrix4d &predict_pose,      // 预测位姿
                                       CloudData::CLOUD_PTR &result_cloud_ptr,   //  转换位姿后的点云
                                       Eigen::Matrix4d &result_pose              // 转换后的位姿
    )
    {
    ...
    size_t ICPSVDRegistration::GetCorrespondence(const CloudData::CLOUD_PTR &input_source,
                                                 std::vector<Eigen::Vector3d> &xs,
                                                 std::vector<Eigen::Vector3d> &ys)
    {
    ...
    void ICPSVDRegistration::GetTransform(const std::vector<Eigen::Vector3d> &xs,
